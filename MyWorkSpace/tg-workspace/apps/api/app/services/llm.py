...
def classify_message_llm(text: str) -> Dict[str, Any]:
    ...
    prompt = f"""Ты - фильтр для вайб-кодера/технического специалиста по автоматизации. 
Проанализируй сообщение и определи, нужно ли это ТЕБЕ как разработчику ботов и автоматизаций.

СООБЩЕНИЕ:
\"\"\"{text[:1500]}\"\"\"

Я - РАЗРАБОТЧИК/КОДЕР. Меня интересуют ТОЛЬКО задачи где нужно ПИСАТЬ КОД или НАСТРАИВАТЬ ТЕХНИЧЕСКУЮ АВТОМАТИЗАЦИЮ:
✅ Telegram/WhatsApp/VK боты (aiogram, pyrogram, salebot, manychat)
✅ Скрипты на Python/JavaScript/Node.js
✅ Автоматизация через n8n, make, zapier
✅ Интеграции API, вебхуки
✅ Парсинг данных, скрапинг
✅ AI/GPT интеграции, нейросети, ChatGPT боты
✅ Автопостинг, контент-заводы с кодом
✅ CRM интеграции с кодом (не просто работа в CRM!)

❌ ЖЁСТКО ОТКЛОНЯЙ если это:
- Вакансия на "менеджера по продажам" - НЕ tech task даже если упоминается CRM!
- "Куратор", "техподдержка", "ассистент" - это НЕ кодинг!
- SMM, таргетолог, дизайнер, копирайтер - НЕ для кодера
- Работа В GetCourse/Senler (а не разработка ДЛЯ них)
- "Проверка ДЗ", "отчёты в Google-таблицах" вручную - НЕ автоматизация
- "#помогу" - это предложения услуг, а не заказы
- Просто использование софта без разработки

ГЛАВНЫЙ ВОПРОС: Нужно ли писать код или настраивать техническую интеграцию?
Если ДА - is_tech_task=true
Если просто работа с готовым софтом руками - is_tech_task=false
...
{{
    ...
    "tech_keywords": ["ключевое слово 1"],
    "budget_hint": "50000 руб" или null,
    "urgency": "low|medium|high",
    "reason": "ОДНО предложение почему да/нет"
}}"""

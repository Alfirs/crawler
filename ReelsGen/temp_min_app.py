...
async def test_photo_overlay(
    image: UploadFile = File(..., description=""Файл фона для пользовательского шаблона""),
    slides_text: str = Form(..., description=""JSON массив текстов слайдов""),
    templates: str = Form(..., description=""JSON массив фонов""),
    size: str = Form(""1080x1350"", description=""Размер слайда, например 1080x1350""),
    darken: bool = Form(True, description=""Включить авто-затемнение"")
):
    """"""
    ...
    for idx, (text_value, template_name) in enumerate(zip(texts, template_names), start=1):
        if not isinstance(text_value, str) or not text_value.strip():
            raise HTTPException(status_code=400, detail=f""Текст слайда #{idx} пустой"")

        user_prompt = (
            ""Создай короткий текст (1–2 предложения) для Instagram-слайда на тему: ""
            f""{text_value.strip()}""
        )
        try:
            generated_text = await chat_complete(
                ""Ты копирайтер Instagram. Пиши короткие, мотивирующие тексты."",
                user_prompt,
            )
            text_value = generated_text.strip() or text_value.strip()
        except Exception as exc:
            print(f""[test-photo-overlay] chat_complete failed: {exc}"")
